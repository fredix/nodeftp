...
bool CFtpServer::SetListeningPort( unsigned short int ListeningPort )
{
        FtpServerLock.Enter();
        {
            usListeningPort = ListeningPort;
        }
        FtpServerLock.Leave();
        return true;
}
	...
	for( ;; ) { // Main Loop
		...
		if( nCmd == CMD_NONE ) {
			...
			if( pClient->bIsLogged == true ) {
                ...
                if( pFtpServer->GetCheckPassDelay() ) QTest::qSleep( pFtpServer->GetCheckPassDelay() );
        ...
        else if( nCmd == CMD_PASV )
        {
			...
			if( pszCmdArg ) {
                ...
                if( pszPath && stat( pszPath, &st ) != 0 )
                {
                    ...
                    else pClient->SendReply( "250 MKD command successful." );
                }
                else pClient->SendReply( "550 File Already Exists." );
            }
            else pClient->SendReply( "501 Syntax error in arguments." );
			continue;
        ...
        else if( nCmd == CMD_RMD || nCmd == CMD_XRMD )
        {
			...
			if( pszCmdArg ) {
                ...
                if( pszPath && stat( pszPath, &st ) == 0 )
                {
                    ...
                    else pClient->SendReply( "250 Directory deleted successfully." );
                }
                else pClient->SendReply( "550 Directory not found." );
            }
            else pClient->SendReply( "501 Syntax error in arguments." );
			continue;
...
void* CFtpServer::CClientEntry::RetrieveThread( void *pvParam )
{
    ...
    check_file = fdopen(hFile, O_RDONLY | O_BINARY );
			...
			|| ( pTransfer->RestartAt > 0 && lseek( hFile, pTransfer->RestartAt, SEEK_SET ) != -1 ) )
		{
			while( pClient->DataSock != INVALID_SOCKET
				...
				if( pTransfer->eDataMode == ZLIB ) {
                    ...
                    // flogier : eof() does not exist, so I use fdopen which is associate a stream with the existing file descriptor
                    nFlush = feof (check_file) ? Z_FINISH : Z_NO_FLUSH;
